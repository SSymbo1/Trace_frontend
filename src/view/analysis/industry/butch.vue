...
import {getButchData} from "@/api/analysis/butch.js";

const tabBread = ref(bread.butch)
const time = ref([])
const query = ref({
  before: '',
  now: '',
...
const searchData = async () => {
  getButchData(query.value).then(resp => {
    if (resp.code === 200) {
      data.value = resp.data.butch
    }
  }).catch(err => {
  })
}
...
const downloadPDF = () => {
  ...
  htmlPdf(
      query.value.before + "至" + query.value.now + "屠宰数据分析",
...
<template>
  <page_container title="">
    ...
    <el-form
      ...
      <el-row>
        <el-col :span="18">
          <el-form-item label="统计日期" v-if="query.type === 'ButchDay'">
            <el-date-picker
          ...
          <el-form-item label="开始月份" v-if="query.type === 'ButchMonth'">
            <el-date-picker type="month" v-model="query.before" value-format="YYYY-MM-1"></el-date-picker>
          </el-form-item>
          <el-form-item label="结束月份" v-if="query.type === 'ButchMonth'">
          ...
          <el-form-item label="查询类别">
            <el-select style="width: 150px" v-model="query.type">
              <el-option label="月" value="ButchMonth"></el-option>
              <el-option label="日" value="ButchDay"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="6">
