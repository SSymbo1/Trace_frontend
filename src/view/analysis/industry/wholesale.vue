...
const searchData = async () => {
  getBatchData(query.value).then(resp => {
    if (resp.code === 200) {
      data.value = resp.data.batch
    }
  }).catch(err => {
  })
}
...
<template>
  <page_container title="">
    ...
    <el-form
      ...
      <el-row>
        <el-col :span="18">
          <el-form-item label="统计日期" v-if="query.type === 'BatchDay'">
            <el-date-picker
          ...
          <el-form-item label="开始月份" v-if="query.type === 'BatchMonth'">
            <el-date-picker type="month" v-model="query.before" value-format="YYYY-MM-1"></el-date-picker>
          </el-form-item>
          <el-form-item label="结束月份" v-if="query.type === 'BatchMonth'">
          ...
          <el-form-item label="查询类别">
            <el-select style="width: 150px" v-model="query.type">
              <el-option label="月" value="BatchMonth"></el-option>
              <el-option label="日" value="BatchDay"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="6">
