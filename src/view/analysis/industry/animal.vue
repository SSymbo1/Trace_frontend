...
import Enterprise_rank from "@/view/echarts/analysis/enterprise_rank.vue";
import {getAnimalData} from "@/api/analysis/animal.js";

const tabBread = ref(bread.animal)
const time = ref([])
const query = ref({
  before: '',
  now: '',
...
const searchData = async () => {
  getAnimalData(query.value).then(resp => {
    if (resp.code === 200) {
      data.value = resp.data.animal
    }
  }).catch(err => {
  })
}
...
const downloadPDF = () => {
  ...
  htmlPdf(
      query.value.before + "至" + query.value.now + "养殖数据分析",
...
<template>
  <page_container title="">
    ...
    <el-form
      ...
      <el-row>
        <el-col :span="18">
          <el-form-item label="统计日期" v-if="query.type === 'AnimalDay'">
            <el-date-picker
          ...
          <el-form-item label="开始月份" v-if="query.type === 'AnimalMonth'">
            <el-date-picker type="month" v-model="query.before" value-format="YYYY-MM-1"></el-date-picker>
          </el-form-item>
          <el-form-item label="结束月份" v-if="query.type === 'AnimalMonth'">
          ...
          <el-form-item label="查询类别">
            <el-select style="width: 150px" v-model="query.type">
              <el-option label="月" value="AnimalMonth"></el-option>
              <el-option label="日" value="AnimalDay"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="6">
