...
const requestFormData = async () => {
  getProductEditInfo(pid.value).then(resp => {
    if (resp.code === 200) {
...
const submitFormData = async () => {
  ...
  formData.value = {...formData.value, pid: pid.value}
  editProduct(formData.value).then(resp => {
    if (resp.code === 200) {
...
<template>
  <el-drawer title="编辑产品信息" v-model="drawerVisible" size="30%" @close="closeDrawer">
    <el-form
